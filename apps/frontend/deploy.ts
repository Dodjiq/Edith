...
import { AwsRegion, deployFunction, deploySite, getOrCreateBucket } from '@remotion/lambda';
...
if (!process.env.REMOTION_AWS_ACCESS_KEY_ID) {
  ...
  console.log('');
  console.log('📖 Complete setup guide: https://www.remotion.dev/docs/editor-starter/rendering');
  process.exit(0);
}

if (!process.env.REMOTION_AWS_SECRET_ACCESS_KEY) {
  console.log('The environment variable "REMOTION_AWS_SECRET_ACCESS_KEY" is not set.');
  console.log('Lambda renders were not set up.');
  console.log('');
  console.log('To set up AWS Lambda rendering:');
  console.log('1. Copy .env.example to .env');
...
if (!awsRegion) {
  ...
  console.log('Please set it to your preferred AWS region (e.g., "us-east-1").');
  console.log('');
  console.log(
