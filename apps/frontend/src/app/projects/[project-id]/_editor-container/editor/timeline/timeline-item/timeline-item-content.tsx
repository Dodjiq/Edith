...
import { FILMSTRIP_HEIGHT_IF_THERE_IS_AUDIO, TimelineItemFilmStrip } from './timeline-item-film-strip';
...
const shouldShowAudioWaveform = ({ item, asset }: { item: EditorStarterItem; asset: EditorStarterAsset | null }) => {
  if (!FEATURE_WAVEFORM) {
    return false;
  }
...
export const TimelineItemContent = memo(
  ({
    ...
    const filmstripHeight =
      asset?.type === 'video' && !asset.hasAudioTrack ? height : FILMSTRIP_HEIGHT_IF_THERE_IS_AUDIO;
      ...
      <>
        <div className="absolute h-full w-full">
          ...
          {isDev ? (
            <div
              aria-hidden
              className="pointer-events-none absolute left-1 top-1 z-10 rounded bg-black/30 px-1 py-0.5 font-mono text-[10px] text-white/40"
            >
              {item.id}
            </div>
          ) : null}
          ...
          {getCanFadeVisual(item) && FEATURE_VISUAL_FADE_CONTROL && (
            <FadeCurve
              item={item}
              height={item.type === 'video' ? filmstripHeight : height - TRACK_PADDING - TIMELINE_ITEM_BORDER_WIDTH * 2}
              width={width}
              fadeType="visual"
            />
          )}
          {asset && shouldShowAudioWaveform({ item, asset }) ? (
            <RequireCachedAsset asset={asset}>
              ...
              >
                {FEATURE_TIMELINE_VOLUME_CONTROL ? (
                  <TimelineItemVolumeControl item={item as AudioItem | VideoItem} />
                ) : null}
              </TimelineItemWaveform>
            </RequireCachedAsset>
          ) : null}
