...
export const updateCaptioningTask = ({
	...
	if (!changed) {
		// Task not found - log warning but still return state
		console.warn(`[updateCaptioningTask] Task not found: ${taskId}. Available tasks:`, state.captioningTasks.map(t => t.id));
		return state;
	}
