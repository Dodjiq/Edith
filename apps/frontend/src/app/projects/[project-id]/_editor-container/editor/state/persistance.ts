...
export const saveState = (state: UndoableState, assetStatus: Record<string, AssetState>) => {
  if (!FEATURE_SAVE_BUTTON) {
    throw new Error('Save button feature flag is disabled');
  }
  ...
  if (assetsUploading) {
    throw new Error('Cannot save while assets are getting uploaded to the cloud');
  }

  if (hasAssetsWithErrors(assetStatus)) {
    throw new Error('Cannot save: Some assets have errors. Please fix them before saving.');
    return;
  }
