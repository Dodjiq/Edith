...
import { CopyButton, type CopyButtonProps } from '@/components/buttons/Copy';
import {
  ButtonGroup,
  ButtonGroupText,
} from '@/app/[locale]/projects/[project-id]/_chatbot/chatbot-components/ai-elements/message/button-group';
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
  TooltipTrigger,
} from '@/app/[locale]/projects/[project-id]/_chatbot/chatbot-components/ai-elements/message/tooltip';
...
export const MessageContent = ({ children, className, ...props }: MessageContentProps) => (
  <div
    className={cn(
      ...
      'group-[.is-user]:text-foreground group-[.is-user]:ml-auto group-[.is-user]:rounded-lg group-[.is-user]:bg-[#28282e] group-[.is-user]:px-4 group-[.is-user]:py-3',
...
export type MessageCopyActionProps = Omit<CopyButtonProps, 'content'> & {
  tooltip?: string;
  label?: string;
  content: string;
};
...
export const MessageCopyAction = ({
  ...
  const { 'aria-label': ariaLabelProp, ...restProps } = copyButtonProps;
  const accessibleLabel = ariaLabelProp ?? label ?? tooltip ?? 'Copy';

  const button = (
    <CopyButton aria-label={accessibleLabel} content={content} size={size} variant={variant} {...restProps} />
  );
...
export const MessageBranchContent = ({ children, ...props }: MessageBranchContentProps) => {
  ...
  useEffect(() => {
    if (childrenArray.length > 0 && branches.length !== childrenArray.length) {
...
export function MessageAttachment({ data, className, onRemove, ...props }: MessageAttachmentProps) {
  ...
  return (
    <div className={cn('group relative size-24 overflow-hidden rounded-lg', className)} {...props}>
      {isImage ? (
        <>
          <Image alt={filename || 'attachment'} className="object-cover" fill sizes="96px" src={data.url} />
          {onRemove && (
            <Button
