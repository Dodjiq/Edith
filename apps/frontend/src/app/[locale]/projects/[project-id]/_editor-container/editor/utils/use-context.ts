...
export const useTimelineSnappingEnabled = () => {
  const context = useContext(TimelineSnappingEnabledContext);
  if (context === null) {
...
export const useSelectedItemsWithTracks = (): SelectedTimelineItem[] => {
  const { selectedItems } = useSelectedItems();
  const { tracks } = useTracks();

  const itemTrackMap = useMemo(() => {
    const map = new Map<string, { track: TrackType; trackIndex: number }>();
    tracks.forEach((track, trackIndex) => {
      track.items.forEach((itemId) => {
        map.set(itemId, { track, trackIndex });
      });
    });
    return map;
  }, [tracks]);
  ...
  return useMemo(
    () =>
      selectedItems.map((itemId) => {
        const entry = itemTrackMap.get(itemId);
        return {
          itemId,
...
export const useOptionalAssetFromAssetId = (assetId: string): EditorStarterAsset | null => {
  const { assets } = useAssets();
  const asset = findAssetById(assets, assetId);
  return asset ?? null;
};

export const useAssetIfApplicable = (item: EditorStarterItem): EditorStarterAsset | null => {
  const { assets } = useAssets();
...
export const useAssetFromItem = (item: EditorStarterItem): EditorStarterAsset => {
...
export const useEditorInitialized = () => {
  const initialized = useContext(StateInitializedContext);
  if (initialized === null) {
    throw new Error('StateInitializedContext is not set');
  }
  return initialized;
};
