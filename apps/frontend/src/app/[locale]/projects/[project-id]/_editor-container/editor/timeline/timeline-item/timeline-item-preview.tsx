...
const ImageItemPreview: React.FC = () => {
  return <SimplePreview style={{ background: ITEM_COLORS.image }}></SimplePreview>;
};
...
const VideoItemPreview: React.FC = () => {
  return <SimplePreview style={{ background: 'var(--color-editor-starter-panel)' }}></SimplePreview>;
};
...
const MotionDesignItemPreview: React.FC<{
  item: MotionDesignItem;
}> = ({ item }) => {
  return (
    <SimplePreview style={{ background: ITEM_COLORS['motion-design'] }}>
      <SimplePreview.Text>{item.templateId}</SimplePreview.Text>
    </SimplePreview>
  );
};
...
export const TimelineItemPreview: React.FC<{
  ...
  if (item.type === 'motion-design') {
    return <MotionDesignItemPreview item={item} />;
  }
