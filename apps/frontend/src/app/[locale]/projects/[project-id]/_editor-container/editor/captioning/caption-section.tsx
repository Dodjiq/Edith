...
import { getAssetDurationInSeconds, getAssetStartInSeconds, getDurationInSecondsOfCaptionAsset } from '../assets/utils';
...
import { MAX_DURATION_ALLOWING_CAPTIONING_IN_SEC } from './audio-buffer-to-mp3';
import { getCaptions } from './caption-state';

const DEFAULT_HIGHLIGHT_COLOR = '#39E508';

export const GenerateCaptionSection: React.FC<{ item: VideoItem | AudioItem }> = ({ item }) => {
  ...
  const caption = useCallback(async () => {
    try {
      ...
      if (asset.type !== 'audio' && asset.type !== 'video') {
        throw new Error('Captioning is only supported for audio and video assets');
      }
      ...
      const videoTrackIndex = tracks.findIndex((track) => track.items.some((i) => i === item.id));
          ...
          return addItem({
            ...
            item: {
              ...
              durationInFrames: Math.round(
                (getDurationInSecondsOfCaptionAsset(captionAsset) - (startInSeconds ?? 0)) * fps,
              ),
              from: item.from,
      ...
      toast.error('Captioning failed', { description: message });
      taskIndicatorRef.current?.open();
    }
  }, [asset, compositionWidth, fps, item, setState, src, tracks]);

  const existingCaptioningTask = captionState.find((task) => task.assetId === asset.id);

  const duration = getAssetDurationInSeconds(asset);

  const isOverLimit = duration === null ? true : duration > MAX_DURATION_ALLOWING_CAPTIONING_IN_SEC;

  const captioningDone = existingCaptioningTask && existingCaptioningTask.status.type === 'done';
  ...
  return (
    <InspectorSection>
      <InspectorLabel>Captions</InspectorLabel>
      <div className="h-2"></div>
      {captioningDone ? <div className="mb-2 text-xs text-neutral-400">Captions already generated</div> : null}

      {isOverLimit ? (
        <div className="mb-2 text-xs italic text-neutral-400">Audio is too long to caption</div>
      ) : (
