...
import { reportMissingRemoteAsset } from './asset-download-failure-store';
...
const pendingCheckIfAssetUrlIsStillValid: Record<string, Promise<void> | undefined> = {};
const missingRemoteAssetErrorMessage = 'Cannot download asset: file not found at remote URL';

const handleAssetDownloadError = (asset: EditorStarterAsset, error: unknown) => {
  if (!(error instanceof Error)) {
    return;
  }

  if (error.message !== missingRemoteAssetErrorMessage) {
    return;
  }

  reportMissingRemoteAsset(asset);
};
...
const checkIfLocalUrlIsStillValid = async (localUrl: string, asset: EditorStarterAsset) => {
...
export const periodicallyCheckIfLocalUrlIsStillValid = async (localUrl: string, asset: EditorStarterAsset) => {
  ...
  pendingCheckIfAssetUrlIsStillValid[asset.id] = checkIfLocalUrlIsStillValid(localUrl, asset);
...
export const loadToBlobUrlOnce = (asset: EditorStarterAsset) => {
  ...
  opens[asset.id] = loadFromAssetToLocalUrl(asset)
    .catch((error) => {
      handleAssetDownloadError(asset, error);
      throw error;
    })
    .finally(() => {
      delete opens[asset.id];
    });
  return opens[asset.id];
};
...
export const useLocalUrls = () => {
  return useSyncExternalStore(
    (cb) => {
      ...
      return () => {
        subscribeToChangeCallbacks = subscribeToChangeCallbacks.filter((fn) => fn !== cb);
      };
    },
    getLocalUrls,
    getLocalUrls,
...
export const useDownloadProgress = (assetId: string) => {
  return useSyncExternalStore(
    (cb) => {
      ...
      return () => {
        subscribeToDownloadProgressCallbacks = subscribeToDownloadProgressCallbacks.filter((fn) => fn !== cb);
      };
    },
...
const downloadToCache = async (asset: EditorStarterAsset, assetStatus: AssetState | null): Promise<void> => {
  if (!asset.remoteUrl) {
  ...
  if (!assetStatus) {
    ...
    if (!fileExists) {
      throw new Error(missingRemoteAssetErrorMessage);
    }
  }
...
export const downloadToCacheOnce = (asset: EditorStarterAsset, assetStatus: AssetState | null): Promise<void> => {
  if (localUrls[asset.id]) {
    return Promise.resolve();
  }
  ...
  downloads[asset.id] = downloadToCache(asset, assetStatus)
    ...
    .finally(() => {
      delete downloads[asset.id];
    });
  return downloads[asset.id];
};

export const clearAssetLocalState = async (assetId: string) => {
  ...
  delete downloadProgress[assetId];

  delete opens[assetId];
  delete downloads[assetId];
  delete lastCheckIfAssetUrlIsStillValid[assetId];
  delete pendingCheckIfAssetUrlIsStillValid[assetId];
  ...
  try {
    ...
    // Ignore IndexedDB cleanup failures because the editor state has already been corrected.
  }
};
