...
export const TimelineItemContextMenu: React.FC<{
	...
	onEditWithAi: (args: {targetItemIds: string[]; anchorRect: DOMRect}) => void;
}> = ({item, onEditWithAi}) => {
	...
	const editWithAiAnchorRectRef = useRef<DOMRect | null>(null);

	const handleEditWithAiPointerDown = useCallback(
		(e: React.PointerEvent<HTMLDivElement>) => {
			e.stopPropagation();
			editWithAiAnchorRectRef.current = e.currentTarget.getBoundingClientRect();
		},
		[],
	);

	const handleEditWithAiFocus = useCallback(
	...
	const handleEditWithAi = useCallback(
		(e: Event) => {
			e.stopPropagation();
			const anchorRect = editWithAiAnchorRectRef.current;
			if (!anchorRect) {
				return;
			}

			onEditWithAi({
				targetItemIds: targetItems.map((targetItem) => targetItem.id),
				anchorRect,
			});
		},
		[onEditWithAi, targetItems],
	);
	...
	const handleContextMenuPointerDown = useCallback(
		...
		<>
			<ContextMenuItem
				...
				onPointerDown={handleEditWithAiPointerDown}
				onFocus={handleEditWithAiFocus}
			>
				Edit with AI
			</ContextMenuItem>
