...
export const getTasksLabel = (tasks: Task[]): { label: React.ReactNode; progress: ProgressType } => {
  ...
  const assetTranscribingTasks = tasks.filter((task) => task.type === 'transcribing');
  const assetDownloadTasks = tasks.filter((task) => task.type === 'downloading');

  const failedRenderTasks = renderTasks.filter((task) => task.status.type === 'error');
  const failedCaptioningTasks = captioningTasks.filter((task) => task.status.type === 'error');
  ...
  const inProgressRenderTasks = renderTasks.filter(
    (task) => task.status.type === 'in-progress' || task.status.type === 'render-initiated',
  );
  const inProgressCaptioningTasks = captioningTasks.filter((task) => task.status.type !== 'done');
  ...
  if (assetTranscribingTasks.length > 0) {
    if (assetTranscribingTasks.length > 1) {
      return {
        label: (
          <>
            Processing {assetTranscribingTasks.length} files
            <DevelopingEllipsis />
          </>
        ),
        progress: { type: 'working' },
  ...
  if (inProgressRenderTasks.length > 0 && inProgressCaptioningTasks.length > 0) {
    return {
      label: (
        <>
  ...
  if (mostRecentTask.type === 'transcribing') {
    return { label: 'Processing completed', progress: { type: 'success' } };
  }

  if (mostRecentTask.type === 'uploading') {
    return { label: 'Upload completed', progress: { type: 'success' } };
  }

  if (mostRecentTask.type === 'downloading') {
    return { label: 'Download completed', progress: { type: 'success' } };
  }
