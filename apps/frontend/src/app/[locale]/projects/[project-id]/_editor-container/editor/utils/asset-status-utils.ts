...
import { checkFileExists } from './assets';
 ...
 */
export const getAssetWithStatus = (state: EditorState, assetId: string): AssetWithStatus | null => {
 ...
 */
export const getAllAssetsWithStatus = (state: EditorState): Record<string, AssetWithStatus> => {
 ...
 */
export const getAssetStatus = (state: EditorState, assetId: string): AssetState | null => {
...
export const createAssetStatusFromUndoableState = async (
  ...
  // Use libraryAssets (superset) to include both timeline and library-only assets
  for (const [assetId, asset] of Object.entries(state.libraryAssets)) {
    if (asset.remoteUrl) {
...
export const hasAssetsWithErrors = (assetStatus: Record<string, AssetState>): boolean => {
