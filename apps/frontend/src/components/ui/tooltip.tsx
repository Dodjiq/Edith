...
} from '@/components/primitives/base/BaseTooltip';
...
function TooltipTrigger({ asChild, ...props }: TooltipTriggerProps) {
  return <TooltipTriggerPrimitive asChild={asChild} {...props} />;
}
    ...
    <TooltipPortalPrimitive>
      <TooltipPositionerPrimitive sideOffset={sideOffset} className="z-50" {...props}>
        <TooltipPopupPrimitive
          className={cn(
            'bg-primary text-primary-foreground origin-(--transform-origin) w-fit text-balance rounded-md px-3 py-1.5 text-xs',
            className,
          )}
          style={style}
        >
...
export {
  ...
  TooltipTrigger,
  TooltipPanel,
  TooltipPanel as TooltipContent,
  type TooltipProps,
