...
|---|---|---|---|---|---|
| Monorepo | `package.json`, `pnpm-workspace.yaml`, `turbo.json` | Organisation PNPM/Turbo | KEEP | Stable et utile pendant la migration | Garder; build Vercel via `pnpm --filter frontend build` |
...
| Server Nest | `apps/server` | API, tools, S3, transcription, render | REPLACE_NOW | Vercel API légère cible | Ne plus dépendre du serveur pour MVP |
...
| Cloudflare config legacy | `wrangler.toml`, `pages:build` | Ancien build Pages frontend | REMOVE_LATER | Déploiement migré sur Vercel | Conserver commenté en legacy, supprimer après stabilisation |
